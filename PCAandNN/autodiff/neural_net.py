...
class Neuron(Module):
    ...
    def __call__(self, x: List[Scalar]) -> Scalar:
        ...
        sum = Scalar(0)
        for input, weight in zip(x, self.w):
           sum_temp = input.__mul__(weight)
           sum = sum.__add__(sum_temp)
        sum = sum.__add__(self.b)
        # z = sum((wi * xi for wi, xi in zip(self.w, x)), self.b)
        return sum.relu() if self.use_relu else sum
...
class MultiLayerPerceptron(Module):
    def __init__(self, num_inputs: int, num_hidden: List[int], num_outputs: int):
        ...
        self.layers = [FeedForwardLayer(layer_sizes[i], layer_sizes[i+1], use_relu=(i < (len(layer_sizes) - 2))) for i in range(len(layer_sizes) - 1)]
